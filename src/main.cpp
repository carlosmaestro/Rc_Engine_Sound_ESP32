...
#ifndef PWM_CHANNELS_INIT
#define PWM_CHANNELS_INIT {1, 2, 3, 4, 5, 6} // Channel numbers
#endif
#ifndef PWM_PINS_INIT
#define PWM_PINS_INIT {13, 12, 14, 27, 35, 34} // Input pin numbers (pin 34 & 35 only usable as inputs!)
#endif
const uint8_t PWM_CHANNELS[PWM_CHANNELS_NUM] = PWM_CHANNELS_INIT;
const uint8_t PWM_PINS[PWM_CHANNELS_NUM] = PWM_PINS_INIT;
...
// Limit required to prevent controller from crashing @ high engine RPM (was 350, but sometimes crashing)
...
// Current RC signal RAW pulse width with linearity compensation [X] = channel number
...
// Crawler mode intended for crawling competitons (withouth sound and virtual inertia)
...
const char *RESET_REASONS[] = {
    "POWERON_RESET", "NO_REASON", "SW_RESET", "OWDT_RESET", "DEEPSLEEP_RESET", "SDIO_RESET", "TG0WDT_SYS_RESET",
    "TG1WDT_SYS_RESET", "RTCWDT_SYS_RESET", "INTRUSION_RESET", "TGWDT_CPU_RESET", "SW_CPU_RESET", "RTCWDT_CPU_RESET",
    "EXT_CPU_RESET", "RTCWDT_BROWN_OUT_RESET", "RTCWDT_RTC_RESET"
};
...
void IRAM_ATTR variablePlaybackTimer() {
    ...
    // Input signals for mixer: a = engine, b = additional sound, c = turbo sound, d = fan sound, e = supercharger sound
    ...
    switch (engineState) {
        ...
        case RUNNING: // Engine running ------------------------------------------------------------------
            ...
            if (!engineJakeBraking && !blowoffTrigger) {
                ...
                a3 = (jakeBrakeSamples[curJakeBrakeSample] * rpmDependentJakeBrakeVolume / 100 *
                      jakeBrakeVolumePercentage / 100); // Jake brake sound
                a2 = 0;
                a1 = 0;
            ...
            if (curChargerSample < chargerSampleCount - 1) {
                e = (chargerSamples[curChargerSample] * throttleDependentChargerVolume / 100 * chargerVolumePercentage /
                     100);
                curChargerSample++;
            } else {
                curChargerSample = 0;
            ...
            if (curHydraulicPumpSample < hydraulicPumpSampleCount - 1) {
                f = (hydraulicPumpSamples[curHydraulicPumpSample] * hydraulicPumpVolumePercentage / 100 *
                     hydraulicPumpVolume / 100);
            ...
            if (curTrackRattleSample < trackRattleSampleCount - 1) {
                g = (trackRattleSamples[curTrackRattleSample] * trackRattleVolumePercentage / 100 * trackRattleVolume /
                     100);
    ...
    uint8_t value = constrain(
        ((a * 8 / 10) + (b / 2) + (c / 5) + (d / 5) + (e / 5) + f + g) * masterVolume / 100 + dacOffset, 0, 255);
...
void IRAM_ATTR fixedPlaybackTimer() {
    ...
    if (wastegateTrigger) {
        if (curWastegateSample < wastegateSampleCount - 1) {
            b3 = (wastegateSamples[curWastegateSample] * rpmDependentWastegateVolume / 100 * wastegateVolumePercentage /
                  100);
            curWastegateSample++;
        } else {
    ...
    if (curDieselKnockSample < knockSampleCount) {
        ...
        b7 = (knockSamples[curDieselKnockSample] * dieselKnockVolumePercentage / 100 * throttleDependentKnockVolume /
              100 * rpmDependentKnockVolume / 100);
#elif defined EXCAVATOR_MODE // knock volume also depending on hydraulic load
        b7 = (knockSamples[curDieselKnockSample] * dieselKnockVolumePercentage / 100 * throttleDependentKnockVolume /
              100 * hydraulicDependentKnockVolume / 100);
    ...
    if (curHydraulicFlowSample < hydraulicFlowSampleCount - 1) {
        c1 = (hydraulicFlowSamples[curHydraulicFlowSample] * hydraulicFlowVolumePercentage / 100 * hydraulicFlowVolume /
              100);
...
void setupEspNow() {
    ...
    if (useTrailer1) {
        ...
        Serial.printf("Trailer 1 MAC address: %02X:%02X:%02X:%02X:%02X:%02X\n", broadcastAddress1[0],
                      broadcastAddress1[1], broadcastAddress1[2], broadcastAddress1[3], broadcastAddress1[4],
                      broadcastAddress1[5]);
    ...
    if (useTrailer2) {
        ...
        Serial.printf("Trailer 2 MAC address: %02X:%02X:%02X:%02X:%02X:%02X\n", broadcastAddress2[0],
                      broadcastAddress2[1], broadcastAddress2[2], broadcastAddress2[3], broadcastAddress2[4],
                      broadcastAddress2[5]);
        if (esp_now_add_peer(&peerInfo) != ESP_OK) {
    ...
    if (useTrailer3) {
        ...
        Serial.printf("Trailer 3 MAC address: %02X:%02X:%02X:%02X:%02X:%02X\n", broadcastAddress3[0],
                      broadcastAddress3[1], broadcastAddress3[2], broadcastAddress3[3], broadcastAddress3[4],
                      broadcastAddress3[5]);
        if (esp_now_add_peer(&peerInfo) != ESP_OK) {
...
#define CELL_SETPOINT (CUTOFF_VOLTAGE - ((FULLY_CHARGED_VOLTAGE - CUTOFF_VOLTAGE) / 2))
    ...
    if (numberOfCells > 1 && numberOfCells < 4) {
        ...
        Serial.printf("Number of cells: %i (%iS battery detected) Based on setpoint: %.2f V\n", numberOfCells,
                      numberOfCells, (CELL_SETPOINT * numberOfCells));
        Serial.printf("Battery cutoff voltage: %.2f V (%i * %.2f V) \n", batteryCutoffvoltage, numberOfCells,
                      CUTOFF_VOLTAGE);
...
void setup() {
    ...
    for (uint8_t coreNum = 0; coreNum < 2; coreNum++) {
        ...
        if (resetReason <= (sizeof(RESET_REASONS) / sizeof(RESET_REASONS[0]))) {
            Serial.printf("Core %i reset reason: %i: %s\n", coreNum, rtc_get_reset_reason(coreNum),
                          RESET_REASONS[resetReason - 1]);
        }
    }
    Serial.printf(
        "**************************************************************************************************\n\n");
    ...
    Serial.printf(
        "ESC pulse span: %i (Used to adjust the top speed: 500 = full ESC power, 1000 = half ESC power etc.)\n",
        escPulseSpan);
    Serial.printf(
        "ESC takeoff punch: %i (Usually 0. Enlarge it up to about 150, if your motor is too weak around neutral.)\n",
        escTakeoffPunch);
    ...
    // begin PPM communication with compatible receivers
    setupMcpwm(); // mcpwm servo output setup
...
#undef NEOPIXEL_ON_CH4 // not usable, pin is required as an input
    ...
    // timer 0, MWDT clock period = 12.5 ns * TIMGn_Tx_WDT_CLK_PRESCALE -> 12.5 ns * 20 -> 250 ns = 0.25 us, countUp
    ...
    // timer 1, MWDT clock period = 12.5 ns * TIMGn_Tx_WDT_CLK_PRESCALE -> 12.5 ns * 20 -> 250 ns = 0.25 us, countUp
    ...
    for (uint8_t i = 1; i < PULSE_ARRAY_SIZE; i++) {
        pulseZero[i] = 1500;
        // Always 1500. This is the center position. Auto centering is now done in "processRawChannels()"
...
void readSbusCommands() {
    ...
    if (!failSafe) {
        ...
        // CH2 3 position switch for gearbox (left throttle in tracked mode)
        pulseWidthRaw[3] = map(SBUSchannels[THROTTLE - 1], 172, 1811, 1000, 2000); // CH3 throttle & brake
        pulseWidthRaw[4] = map(SBUSchannels[HORN - 1], 172, 1811, 1000, 2000);
        // CH5 jake brake, high / low beam, headlight flasher, engine on / off
        pulseWidthRaw[5] = map(SBUSchannels[FUNCTION_R - 1], 172, 1811, 1000, 2000);
        // CH5 jake brake, high / low beam, headlight flasher, engine on / off
...
void loopIbus() {
    ...
    if (millis() - lastIbusRead > 10) {
        ...
        if (iBusReadCycles < 100)
            ...
            ibusInit = true;
        // We need to process the entire serial package, before we read the channels for the first time! 100 OK? TODO
    }
}
...
void readIbusCommands() {
    ...
    pulseWidthRaw[2] = iBus.readChannel(GEARBOX - 1);
    // CH2 3 position switch for gearbox (left throttle in tracked mode)
    pulseWidthRaw[3] = iBus.readChannel(THROTTLE - 1); // CH3 throttle & brake
    pulseWidthRaw[4] = iBus.readChannel(HORN - 1);
    // CH5 jake brake, high / low beam, headlight flasher, engine on / off
    pulseWidthRaw[5] = iBus.readChannel(FUNCTION_R - 1);
    // CH5 jake brake, high / low beam, headlight flasher, engine on / off
...
void readSumdCommands() {
    ...
    pulseWidthRaw[2] = map(SUMDchannels[GEARBOX - 1], 1100, 1900, 1000, 2000);
    // CH2 3 position switch for gearbox (left throttle in tracked mode)
    pulseWidthRaw[3] = map(SUMDchannels[THROTTLE - 1], 1100, 1900, 1000, 2000); // CH3 throttle & brake
    pulseWidthRaw[4] = map(SUMDchannels[HORN - 1], 1100, 1900, 1000, 2000);
    // CH5 jake brake, high / low beam, headlight flasher, engine on / off
    pulseWidthRaw[5] = map(SUMDchannels[FUNCTION_R - 1], 1100, 1900, 1000, 2000);
    // CH5 jake brake, high / low beam, headlight flasher, engine on / off
...
void processRawChannels() {
    ...
    if (millis() - lastOutOfRangeMillis > 500) {
        for (uint8_t i = 1; i < PULSE_ARRAY_SIZE; i++) {
            ...
            if (channelAutoZero[i] && !autoZeroDone && pulseWidthRaw3[i] > pulseMinValid && pulseWidthRaw3[i] <
                pulseMaxValid)
...
#undef TRANSMISSION_NEUTRAL // Not usable in this case!
        ...
        unsigned long rampsDelayMicrosTarget = map(abs(winchSpeed), 1, 100, 30000, 2000);
        // Percentage to delay 1, 100, 30000, 2000
    ...
    if (millis() - printServoMillis > 1000) {
        ...
        Serial.printf("CH1 (steering) :  %i µS, %.2f°\n", steeringServoMicrosDelayed,
                      us2degree(steeringServoMicrosDelayed));
...
void eepromInit() {
    if (EEPROM.read(adr_eprom_init) != eeprom_id) {
        ...
        EEPROM.write(adr_eprom_hazardsWhile5thWheelUnlocked, hazardsWhile5thWheelUnlocked);
        // Hazards on, if 5th wheel unlocked
        ...
        EEPROM.writeUShort(adr_eprom_rearLightDimmedBrightness, rearlightDimmedBrightness);
        // Taillight brightness, if not braking. About 30
        EEPROM.writeUShort(adr_eprom_tailLightParkingBrightness, rearlightParkingBrightness);
        // Taillight brightness, if sidelights only are on. 3 - 5, 0 for US mode
        EEPROM.writeUShort(adr_eprom_headLightParkingBrightness, headlightParkingBrightness);
        // Headlight brightness, if sidelights only are on. 3 - 5, 0 for US mode
        EEPROM.writeUShort(adr_eprom_sideLightBrightness, sideLightsBrightness);
        // Sidelights brightness about 100 - 200
        EEPROM.writeUShort(adr_eprom_reversingLightBrightness, reversingLightBrightness);
        // Reversing lights brightness about 140
        ...
        EEPROM.writeUShort(adr_eprom_global_acceleration_percentage, globalAccelerationPercentage);
        // ESC acceleration percentage
...
void eepromRead() {
    ...
    hazardsWhile5thWheelUnlocked = EEPROM.read(adr_eprom_hazardsWhile5thWheelUnlocked);
    // Hazards on, if 5th wheel unlocked
    ...
    rearlightParkingBrightness = EEPROM.readUShort(adr_eprom_tailLightParkingBrightness);
    // Taillight brightness, if sidelights only are on. 3 - 5, 0 for US mode
    headlightParkingBrightness = EEPROM.readUShort(adr_eprom_headLightParkingBrightness);
    // Headlight brightness, if sidelights only are on. 3 - 5, 0 for US mode
    sideLightsBrightness = EEPROM.readUShort(adr_eprom_sideLightBrightness); // Sidelights brightness about 100 - 200
    reversingLightBrightness = EEPROM.readUShort(adr_eprom_reversingLightBrightness);
    // Reversing lights brightness about 140
    ...
    globalAccelerationPercentage = EEPROM.readUShort(adr_eprom_global_acceleration_percentage);
    // ESC acceleration percentage
...
void mapThrottle() {
    ...
    if (micros() - throttleFaderMicros > 500) {
        ...
        if (!escIsBraking && !brakeDetect && engineRunning)
            throttleDependentVolume = map(currentThrottleFaded, 0, 500, engineIdleVolumePercentage,
        ...
        if (!escIsBraking && !brakeDetect && engineRunning)
            throttleDependentRevVolume = map(currentThrottleFaded, 0, 500, engineRevVolumePercentage,
                                             fullThrottleVolumePercentage);
        ...
        if (!escIsBraking && !brakeDetect && engineRunning && (currentThrottleFaded > dieselKnockStartPoint))
            throttleDependentKnockVolume = map(currentThrottleFaded, dieselKnockStartPoint, 500,
                                               dieselKnockIdleVolumePercentage, 100);
        ...
        if (!escIsBraking && !brakeDetect && engineRunning && (currentRpm > chargerStartPoint))
            throttleDependentChargerVolume = map(currentThrottleFaded, chargerStartPoint, 500,
                                                 chargerIdleVolumePercentage, 100);
        else
...
void engineMassSimulation() {
    ...
    if (millis() - throtMillis > timeBase) {
        ...
        if ((currentSpeed < clutchEngagingPoint && _currentRpm < maxClutchSlippingRpm) || gearUpShiftingInProgress ||
            gearDownShiftingInProgress || neutralGear) {
            clutchDisengaged = true;
        } else {
        ...
        if (automatic) {
            ...
            if (selectedAutomaticGear < 2)
                ...
                // more slip in first and reverse gear
            else
        ...
        // Accelerate engine
        if (targetRpm > (_currentRpm + acc) && (_currentRpm + acc) < maxRpm && engineState == RUNNING &&
            engineRunning) {
            if (!airBrakeTrigger) {
    ...
    blowoffTrigger = ((gearUpShiftingInProgress || neutralGear) && millis() - blowoffMillis > 20 && millis() -
                      blowoffMillis < 250);
#endif
...
void brakeLightsSub(uint8_t brightness) {
    if (escIsBraking) {
        ...
        tailLight.pwm(constrain(brightness - (crankingDim / 2), (brightness / 2), 255));
        // Taillights (reduced brightness)
        brakeLight.pwm(constrain(brightness - (crankingDim / 2), (brightness / 2), 255));
        // Brakelight (reduced brightness)
    }
}
...
void headLightsSub(bool head, bool fog, bool roof, bool park) {
    ...
    } else // Bulbs wired as labeled on the board ----
    {
        ...
        if (!head && !park) {
            ...
            if (!headLightsFlasherOn)
                headLight.pwm(
                    constrain(headlightParkingBrightness - crankingDim, (headlightParkingBrightness / 2), 255));
            else
                headLight.on();
            xenonMillis = millis();
            headLightsHighBeamOn = false;
...
void led() {
    ...
    if (!hazard && !unlock5thWheel && !batteryProtection && hazardsWhile5thWheelUnlocked || !hazard && !
        batteryProtection && !hazardsWhile5thWheelUnlocked) {
        // L indicator
        if (indicatorLon) {
...
void gearboxDetection() {
    ...
    if (!crawlerMode) {
        ...
        if (currentRpm > 490 && selectedGear == 3 && engineLoad < 5 && currentThrottle > 490 && millis() -
            lastShiftingMillis > 2000) {
            overdrive = true;
        }
        if (!escIsBraking) {
    ...
    if (currentRpm > 490 && selectedGear < 3 && engineLoad < 5 && currentThrottle > 490 && millis() - lastShiftingMillis
        > 2000) {
        selectedGear++;
    }
    if (!escIsBraking) {
...
void automaticGearSelector() {
    ...
    if (millis() - gearSelectorMillis > 100) {
        ...
        if (escInReverse) {
            ...
            if (millis() - lastUpShiftingMillis > 600 && selectedAutomaticGear > 1 && (
                    _currentRpm <= downShiftPoint || engineLoad > 100)) {
...
void esc() {
    ...
    if (automatic || doubleClutch) {
        ...
        // faster acceleration in automatic reverse, EXPERIMENTAL, TODO!
    }
    ...
    if (millis() - escMillis > escRampTime) {
        ...
        switch (driveState) {
            ...
            case 1: // Driving forward ---------------------------------------------------------------------
                ...
                if (pulse() == -1 && escPulse() == 0)
                    driveState = 3;
                // Driving backwards, if ESC not yet moving. Prevents state machine from hanging! v9.7.0
            ...
            case 3: // Driving backwards ---------------------------------------------------------------------
                ...
                if (pulse() == 1 && escPulse() == 0)
                    driveState = 1;
                // Driving forward, if ESC not yet moving. Prevents state machine from hanging! v9.7.0
                if (pulse() == 0 && escPulse() == 0)
                    driveState = 0; // standing still
                break;
        ...
        if (escSignal > 1500) {
            ...
            // MCPWM_DUTY_MODE_1 = inverse PWM mode, high, if 0% PWM
...
void rcTriggerRead() {
    ...
    if (functionR100u.toggleLong(pulseWidth[5], 1000) != beamStateLock) {
        ...
        // This lock is required, because high / low beam needs to be able to be changed in other program sections!
    ...
    if (driveState == 0 && (engineState == OFF || engineState == RUNNING)) {
        ...
        if (functionR75d.toggleLong(pulseWidth[5], 1850) != engineStateLock) {
            engineOn = !engineOn;
            // This lock is required, because engine on / off needs to be able to be changed in other program sections!
...
void updateDashboard() {
    ...
    if (millis() - lastFrameTime > 40) // We need to save processing time!
    {
...
#if defined BATTERY_PROTECTION
            fuel = map_Generic<float, float, float, int16_t, int16_t>((batteryVoltage / numberOfCells), CUTOFF_VOLTAGE,
                                                                      FULLY_CHARGED_VOLTAGE, 0, 100);
            if (fuel > 100)
                fuel = 100;
            if (fuel < 0)
                fuel = 0;
        ...
        if (hazardsWhile5thWheelUnlocked) {
            dashboard.setLeftIndicator(
                indicatorSoundOn && (indicatorLon || hazard || unlock5thWheel || batteryProtection));
            dashboard.setRightIndicator(
                indicatorSoundOn && (indicatorRon || hazard || unlock5thWheel || batteryProtection));
        } else {
...
void excavatorControl() {
    ...
    if (millis() - lastFrameTime > 4) {
        ...
        hydraulicPumpVolumeInternalUndelayed = constrain(
            hydraulicPumpVolumeInternal[1] + hydraulicPumpVolumeInternal[2] + hydraulicPumpVolumeInternal[5] +
        ...
        if (engineRunning)
            trackRattleVolumeInternalUndelayed = constrain(trackRattleVolumeInternal[6] + trackRattleVolumeInternal[7],
                                                           0, 100) * map(currentRpm, 0, 500, 100, 150) / 100;
        else
